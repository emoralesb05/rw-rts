...
// Four-warden system. Each archetype has a primitive silhouette
...
// the user drops into `assets/sprites/rw/wardenN_sheet.png`.
...
export const SPRITE_URL = (role: UnitRole) =>
  publicAsset(`sprites/rw/${role}.png`);
export const TEXTURE_KEY = (role: UnitRole) => `rw-unit-${role}`;

// Shipped default art generated by scripts/generate-pixel-sprites.ts.
export const SPRITE_DEFAULT_URL = (role: UnitRole) =>
  publicAsset(`sprites/rw-default/${role}.png`);
export const TEXTURE_DEFAULT_KEY = (role: UnitRole) => `rw-default-${role}`;

const SKIN = 0xf5cba0;

/**
 * Primitive chibi silhouette used as a last-resort fallback when no
 * sprite is loaded. Four distinguishable archetypes — warden1
 * (Vaelen, dusk purple), warden2 (Selene, dream pink),
 * warden3 (Ryder, forge orange), warden4 (Lyris, tide cyan).
 */
export function drawRWUnit(
  scene: Phaser.Scene,
  role: UnitRole
): Phaser.GameObjects.GameObject[] {
  switch (role) {
    case "warden2":
      return drawWarden2(scene);
    case "warden3":
      return drawWarden3(scene);
    case "warden4":
      return drawWarden4(scene);
    default:
      return drawWarden1(scene);
  }
}

// Masculine warden — dark navy/purple coat, spiky dark hair.
function drawWarden1(scene: Phaser.Scene): Phaser.GameObjects.GameObject[] {
...
// Feminine warden — pale lavender/pink robes, long pink hair.
function drawWarden2(scene: Phaser.Scene): Phaser.GameObjects.GameObject[] {
  const objs: Phaser.GameObjects.GameObject[] = [];
  // Body — flowing robes
  objs.push(
    scene.add
...
// Masculine warden — crimson cloak over dark tunic, copper hair.
function drawWarden3(scene: Phaser.Scene): Phaser.GameObjects.GameObject[] {
  const objs: Phaser.GameObjects.GameObject[] = [];
  // Body — dark tunic
  objs.push(
    scene.add
...
// Feminine warden — cyan tunic over white undershirt, teal hair.
function drawWarden4(scene: Phaser.Scene): Phaser.GameObjects.GameObject[] {
  const objs: Phaser.GameObjects.GameObject[] = [];
  // Body — cyan tunic
  objs.push(
    scene.add
...
export function drawRWBuilding(
  scene: Phaser.Scene,
  kind: "citadel" | "bastion" | "crossroads",
  x: number,
  y: number
): Phaser.GameObjects.GameObject[] {
  const objs: Phaser.GameObjects.GameObject[] = [];
  switch (kind) {
    ...
    case "bastion": {
      ...
      const sealStem = scene.add.rectangle(x, y - 12, 2.5, 7, 0xffd86b);
      objs.push(base, wall, tallSpire, sealRing, sealStem);
      break;
    }
    case "crossroads": {
      const base = scene.add
