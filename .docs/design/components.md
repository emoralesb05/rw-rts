...
`src/renderer/src/components/` is the reusable design-system layer.
`src/renderer/src/ui/` is the product layer: HUDs, panels, drawers,
letters, wielder-specific surfaces, and app workflows.

This split keeps domain concepts from leaking into shared controls.
For example, `Button` belongs in `components/kit`; `AgentToolBadge`
...
| Layer | Path | Purpose |
|---|---|---|
| Primitives | `components/primitives/` | Thin wrappers around behavior/accessibility libraries such as Radix UI and cmdk. |
| Kit | `components/kit/` | Keykeeper-styled reusable controls and atoms: buttons, badges, fields, bars, inputs, toasts. |
...
1. Existing `components/kit` component.
2. Existing `components/primitives` wrapper plus Tailwind classes.
3. A new small `components/kit` component when repetition is real.
