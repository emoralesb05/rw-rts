...
| [gemini-auth-status-2026-06-29.md](gemini-auth-status-2026-06-29.md) | Can this machine run a non-interactive Gemini live policy probe? | No: cached `oauth-personal` auth is configured, but installed `0.47.0` and npm-latest `0.49.0` both fail with `IneligibleTierError` / `UNSUPPORTED_CLIENT`; no API key, Vertex env, `gcloud`, or ADC config is present. | Supported Gemini CLI auth: API key, Vertex/GCA env, or supported account tier |
