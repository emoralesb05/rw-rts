...
| [gemini-auth-status-2026-06-29.md](gemini-auth-status-2026-06-29.md) | Can this machine run a non-interactive Gemini live policy probe? | Blocked: `gemini --list-sessions` reached auth but failed with `IneligibleTierError` / `UNSUPPORTED_CLIENT` for Gemini Code Assist for individuals. | Supported Gemini CLI auth: API key, Vertex/GCA env, or supported account tier |
