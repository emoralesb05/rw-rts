...
- A live Gemini policy execution probe still needs a supported non-interactive auth path. This machine has cached `oauth-personal` auth selected, and headless mode should reuse cached auth, but 2026-06-29 probes with installed `0.47.0` and npm-latest `0.49.0` both failed with `IneligibleTierError` / `UNSUPPORTED_CLIENT` for Gemini Code Assist for individuals. The same pass found no `GEMINI_API_KEY`, `GOOGLE_API_KEY`, Vertex/Google Cloud project env, `GOOGLE_APPLICATION_CREDENTIALS`, `gcloud`, or ADC config, so the next probe needs a throwaway API key, Vertex/GCA env, or a supported Gemini CLI account tier.
