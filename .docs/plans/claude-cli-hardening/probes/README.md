...
| [claude-ask-user-question-live-2026-06-29.md](claude-ask-user-question-live-2026-06-29.md) | Can a bounded live turn validate `PreToolUse` / `AskUserQuestion` answer letters? | Blocked: `--tools AskUserQuestion` initialized with `tools: []`; Claude emitted malformed text instead of a real tool call and then hit the budget cap. | Provider-supported way to expose/trigger `AskUserQuestion` in print mode |
