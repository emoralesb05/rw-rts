...
- **Headless auth differs from status.** On 2026-06-29, `cursor-agent status` reported login success and `create-chat` returned a chat id, but `cursor-agent about` still showed `User Email: Not logged in` and `models` failed. Running `cursor-agent login` and completing browser auth fixed print mode; validate with an API-backed command such as `cursor-agent models`, not `status` alone.
